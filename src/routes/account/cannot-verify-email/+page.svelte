<script>
    ...
    async function validateAndSubmitEmail() {
        ...
        await postJson('/api/user/send-verification-link', { email: email.value }, {
            onSuccess: async (_data) => success(_data?.message ?? 'Link has been sent. You may now check your inbox.'),
            onFail: async (err) => error(err?.message ?? 'Unable to send email verification link due to an error.'),
        });
    }
</script>
...
<BaseContainer withBgImage={true} class="animate-bg">
    <div class="bg-white/50 w-full h-full flex items-center justify-center">
        <RoundedCard class="p-8 pb-6 md:w-2/7 rounded-xl flex flex-col gap-y-5">
            <div class="space-y-2">
                <TextM class='font-bold text-gray-600'>Send Email Verification Link</TextM>
                <TextS>Enter your Etrace+ account's email address so that we can send you an email verification link</TextS>
            </div>
            <InputText
                label='Email'
