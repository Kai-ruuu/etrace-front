<script>
	...
	import RequirementS from "$lib/components/user/RequirementS.svelte";
    ...
    async function onPend() {
        await patchJson('/api/user/company/pend/' + confirmPend.item?.id, 
            { message: rejectionMessage.value },
            {
                ...
                onFinish: () => dataLoading = false,
                onSuccess: async (newData) => {
                    success(confirmPend.item.profile.name + ' has been marked as pending for verification.');
            ...
            <div class="flex flex-col overflow-clip">
                ...
                <div class="flex flex-col gap-y-2 p-4 border-t border-gray-100">
                    ...
                    <div class="flex flex-col gap-y-2">
                        ...
                        <div class="flex items-center gap-x-2">
                            {#if openedProfile.profile.ver_stat_sysad === 'Pending'}
                ...
                <div class="flex flex-col gap-y-2 p-4 border-t border-gray-100">
                    ...
                    <RequirementS profile={openedProfile} reqKey='req_company_profile' reqDisplay='Company Profile'/>
                    <RequirementS profile={openedProfile} reqKey='req_business_permit' reqDisplay='Business Permit'/>
                    <RequirementS profile={openedProfile} reqKey='req_sec' reqDisplay='SEC'/>
                    <RequirementS profile={openedProfile} reqKey='req_dti_cda' reqDisplay='DTI / CDA Reg.'/>
                    <RequirementS profile={openedProfile} reqKey='req_reg_of_est' reqDisplay='Registry of Establishment fr. DOLE'/>
                    <RequirementS profile={openedProfile} reqKey='req_cert_from_dole' reqDisplay='Certification from DOLE Provincial Office'/>
                    <RequirementS profile={openedProfile} reqKey='req_cert_no_case' reqDisplay='Certification of No Pending Case'/>
                    <RequirementS profile={openedProfile} reqKey='req_philjobnet_reg' reqDisplay='Phil-JobNet Reg.'/>
                    <RequirementS profile={openedProfile} reqKey='req_list_of_vacancies' reqDisplay='List of Vacancies'/>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="w-full overflow-auto max-h-full scrollbar">
            ...
            <div class="flex flex-col gap-y-2 p-4 border-t border-gray-100">
                ...
                <div class="flex flex-col gap-y-2">
                    <div class="flex items-center gap-x-2">
                        ...
                        <TextS>Rejected on: <span class="text-gray-600 font-bold">{formatDate(openAppealsProfile.profile.rejection_sysad.created_at)}</span></TextS>
                    </div>
                    <div class="flex flex-col items-stretch">
                        ...
                        <div class="text-sm rounded text-gray-600 ml-6 pt-1">
                            {openAppealsProfile.profile.rejection_sysad.message}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-y-2 p-4 border-t border-gray-100">
                <div class="flex items-center justify-between">
                    ...
                    {#if openAppealsProfile.profile.rejection_sysad.appeal}
                        <button
                            Icon={X}
                            label='Close'
                            onclick={() => {
                ...
                <div class="flex flex-col gap-y-2">
                    {#if openAppealsProfile.profile.rejection_sysad.appeal}
                        <div class="flex items-center gap-x-2">
                            <Calendar class='w-4 text-gray-400'/>
                            <TextS>Appealed on: <span class="text-gray-600 font-bold">{formatDate(openAppealsProfile.profile.rejection_sysad.appeal.created_at)}</span></TextS>
                        </div>
                        <div class="flex flex-col items-stretch">
                            <div class="flex items-center gap-x-2">
                            ...
                            <div class="text-sm rounded text-gray-600 ml-6 pt-1">
                                {openAppealsProfile.profile.rejection_sysad.appeal.message}
                            </div>
                        </div>
                    {:else}
                        ...
                        {#each data.results as company (company.id)}
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors **:text-nowrap">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    {:else if company.profile.ver_stat_sysad === 'Pending'}
                                ...
                                <td class="px-4 py-3 flex items-center gap-x-2">
                                    ...
                                    {#if company.profile.ver_stat_sysad === 'Pending'}
                                        <button
                                            onclick={() => {
                                    ...
                                    {:else if company.profile.ver_stat_sysad === 'Rejected'}
                                        <button
