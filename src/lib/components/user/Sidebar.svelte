<script>
    ...
    function handleResize() {
        ...
        sidebarShown.set(screenType === "desktop");
    }
...
{#if $sidebarShown}
            ...
            <div class="w-full p-4 flex flex-col gap-y-5">
                {#if $user.role === 'sysad'}
                    <SidebarGroup label='Main Menu'>
                        <SidebarLink desktop={true} Icon={LayoutDashboard} label='Dashboard' path='/user/system-admin'/>
                    </SidebarGroup>
                    <SidebarGroup label='Manage'>
                        <SidebarLink desktop={true} Icon={School} label='Schools' path='/user/system-admin/manage-schools'/>
                        {#if $user.default}
                            <SidebarLink desktop={true} Icon={User} label='System Administrators' path='/user/system-admin/manage-system-admins'/>
                        {/if}
                        <SidebarLink desktop={true} Icon={User} label='Deans' path='/user/system-admin/manage-deans'/>
                        <SidebarLink desktop={true} Icon={User} label='PESO Staffs' path='/user/system-admin/manage-peso-staffs'/>
                        <SidebarLink desktop={true} Icon={Building2} label='Companies' path='/user/system-admin/manage-companies'/>
                    </SidebarGroup>
                    <SidebarGroup label='Settings'>
                        <SidebarLink desktop={true} Icon={Settings} label='Settings' path='/user/system-admin/settings'/>
                        <SidebarLogoutButton />
                    </SidebarGroup>
                {:else if $user.role === 'dean'}
                    <SidebarGroup label='Main Menu'>
                        <SidebarLink desktop={true} Icon={LayoutDashboard} label='Dashboard' path='/user/dean'/>
                    </SidebarGroup>
                    <SidebarGroup label='Manage'>
                        <SidebarLink desktop={true} Icon={Book} label='Courses' path='/user/dean/manage-courses'/>
                        <SidebarLink desktop={true} Icon={FileUser} label='Graduate Records' path='/user/dean/manage-records'/>
                        <SidebarLink desktop={true} Icon={Briefcase} label='Occupations' path='/user/dean/manage-occupations'/>
                        <SidebarLink desktop={true} Icon={User} label='Alumni' path='/user/dean/manage-alumni'/>
                    </SidebarGroup>
                    <SidebarGroup label='Settings'>
                        <SidebarLink desktop={true} Icon={Settings} label='Settings' path='/user/dean/settings'/>
                        <SidebarLogoutButton />
                    </SidebarGroup>
                {:else if $user.role === 'pstaff'}
                    <SidebarGroup label='Main Menu'>
                        <SidebarLink desktop={true} Icon={LayoutDashboard} label='Dashboard' path='/user/peso-staff'/>
                    </SidebarGroup>
                    <SidebarGroup label='Manage'>
                        <SidebarLink desktop={true} Icon={User} label='Companies' path='/user/peso-staff/manage-companies'/>
                    </SidebarGroup>
                    <SidebarGroup label='Settings'>
                        <SidebarLink desktop={true} Icon={Settings} label='Settings' path='/user/peso-staff/settings'/>
                        <SidebarLogoutButton />
                    </SidebarGroup>
                {:else if $user.role === 'company'}
                    <SidebarGroup label='Main Menu'>
                        <SidebarLink desktop={true} Icon={Briefcase} label='Job Posts' path='/user/company'/>
                        <SidebarLink desktop={true} Icon={BadgeCheck} label='Verification Center' path='/user/company/verification-center'/>
                    </SidebarGroup>
                    <SidebarGroup label='Settings'>
                        <SidebarLink desktop={true} Icon={Settings} label='Settings' path='/user/company/settings'/>
                        <SidebarLogoutButton />
                    </SidebarGroup>
                {/if}
            </div>
    ...
    {:else}
        <div class="fixed top-0 left-0 w-full h-dvh border-r border-gray-200 bg-white z-100">
            <div class="min-h-16 flex items-center bg-white px-4 gap-x-3">
                <div class="flex items-center justify-center min-w-8 min-h-8 bg-blue-50 border border-blue-100 rounded">
                    <GraduationCap class='min-w-6 text-blue-900'/>
                </div>
